...
# Format code
printf "%s" "$ALL_FILES" | xargs shfmt -l -w || exit 1

# Lint code
printf "%s" "$ALL_FILES" | xargs shellcheck --format diff | git apply --allow-empty || exit 1

# Run again for files that could not be autofixed
printf "%s" "$ALL_FILES" | xargs shellcheck || exit 1
